...
grep -e ^-[1-9] release/songs_$targetTag | sed 's/^-//g' | awk -F"|" '{printf "DELETE FROM songs WHERE sn = %d;\n", $1}' >> $sqlCmd
...
grep -e ^-[1-9] release/artists_$targetTag | sed 's/^-//g' | awk -F"|" '{printf "DELETE FROM artists WHERE sn = %d;\n", $1}' >> $sqlCmd
